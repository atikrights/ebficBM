...
class AssetProvider extends ChangeNotifier {
  ...
  Future<String?> syncUrlToLibrary(String url, {String? name}) async {
    ...
    // For mobile, we just track it locally if it's a web link, 
    // or we could implement a backend 'register' endpoint.
    // For now, let's keep it consistent with Central's placeholder approach.
    final newId = 'ASSET-${100000 + Random().nextInt(899999)}';
    _assets.insert(0, AssetModel(
      id: newId,
      name: name ?? 'Linked Web Asset',
      path: url,
