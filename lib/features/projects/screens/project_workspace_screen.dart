...
class _ProjectWorkspaceScreenState extends State<ProjectWorkspaceScreen> {
      ...
      child: GlassContainer(
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton.icon(
              onPressed: () {
                _showGlobalCreateDialog(context, project, isDark);
              },
  ...
  void _showGlobalCreateDialog(BuildContext context, Project project, bool isDark) {
          ...
          ElevatedButton(
            onPressed: () async {
              if (titleCtrl.text.trim().isNotEmpty) {
                final newId = await context.read<ProjectProvider>().deployProject(
                  name: titleCtrl.text.trim(),
                  companyId: project.companyId ?? '1', // Default to 1 if missing
                );
...
class _PlanConsoleBoardState extends State<_PlanConsoleBoard> {
  ...
  void _updateNodeStatus(SystemTask task, TaskStatus newStatus) {
    ...
    context.read<ProjectProvider>().updatePlanStatus(
      widget.project.id, 
      widget.plan.id, 
      _mapTaskToProjectStatus(newStatus).name
    );
