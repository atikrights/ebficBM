...
class _AttachProjectDialogState extends State<AttachProjectDialog> {
  ...
  Future<void> _loadUnattachedProjects() async {
    ...
    try {
      final list = await Provider.of<ProjectProvider>(context, listen: false).fetchUnattachedProjects();
      if (mounted) {
        setState(() {
  ...
  Future<void> _attachProject(pmod.Project project) async {
    ...
    try {
      final success = await Provider.of<ProjectProvider>(context, listen: false).attachToCompany(project.id, widget.company.id);
      if (success && mounted) {
        setState(() {
  ...
  Future<void> _attachManualPid() async {
    ...
    try {
      final project = await Provider.of<ProjectProvider>(context, listen: false).searchByPid(pid);
      if (project == null) {
        if (mounted) {
      ...
      if (success && mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Failed to link project.'),
            backgroundColor: Colors.red,
          ),
        );
      }
