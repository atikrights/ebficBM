...
class Company {
  ...
  // Stats
  final int activeEmployees; // staff_count
  ...
  factory Company.fromMap(Map<String, dynamic> map) {
    return Company(
      ...
      logoUrl: map['logo_url'] ?? map['logo_path'],
      categories: (map['categories'] is String
          ? (_safeJsonDecode(map['categories']) as List?)?.map((e) => e.toString()).toList()
          : (map['categories'] as List?)?.map((e) => e.toString()).toList()) ?? [],
      website: map['website'] ?? '',
      activeEmployees: (map['staff_count'] ?? map['active_employees'] ?? 0) as int,
      managerCount: (map['manager_count'] ?? 0) as int,
      ...
      // ✅ CRITICAL FIX: backend sends status_text (string), not status (int)
      status: _parseStatus(map['status_text'] ?? map['status']),
      primaryEmail: map['primary_email'] ?? '',
      phone: map['phone'] ?? '',
      location: map['location'] ?? '',
      projectIds: (map['project_ids'] is String
          ? (_safeJsonDecode(map['project_ids']) as List?)?.map((e) => e.toString()).toList()
          : (map['project_ids'] as List?)?.map((e) => e.toString()).toList()) ?? [],
      ...
      managerSignatureTimestamp: map['manager_signature_timestamp'] != null
          ? DateTime.tryParse(map['manager_signature_timestamp'].toString())
          : null,
      founderSignature: map['founder_signature'],
      founderSignatureTimestamp: map['founder_signature_timestamp'] != null
          ? DateTime.tryParse(map['founder_signature_timestamp'].toString())
          : null,
      socialLinks: map['social_links'] is String
          ? Map<String, String>.from(_safeJsonDecode(map['social_links']) ?? {})
          : (map['social_links'] != null
              ? (map['social_links'] as Map).map((k, v) => MapEntry(k.toString(), v?.toString() ?? ''))
              : null),
      onlinePlatforms: (map['online_platforms'] is String
          ? (_safeJsonDecode(map['online_platforms']) as List?)
          : (map['online_platforms'] as List?))
          ?.map((e) {
            final m = e as Map;
  ...
  /// Parses status from either a string ("active","archived") or legacy int index
  static CompanyStatus _parseStatus(dynamic value) {
    if (value == null) return CompanyStatus.active;
    if (value is int) {
      return value >= 0 && value < CompanyStatus.values.length
          ? CompanyStatus.values[value]
          : CompanyStatus.active;
    }
    if (value is String) {
      switch (value.toLowerCase()) {
        case 'archived': return CompanyStatus.archived;
        case 'onhold':
        case 'on_hold': return CompanyStatus.onHold;
        case 'pending': return CompanyStatus.pending;
        case 'declined': return CompanyStatus.declined;
